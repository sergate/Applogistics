...
export async function GET() {
  try {
    const cacheKey = getCacheKey('dashboard', 'por-canal')
    ...
    // By canal + seller
    const detalle = await prisma.pedidoProcesado.groupBy({
      by: ['canal', 'seller'],
      _sum: {
        unidades: true,
        unidadesPickeadas: true,
    ...
    const detalleResult = (detalle ?? []).map((d: any) => {
      const uni = d?._sum?.unidades ?? 0
      const pick = d?._sum?.unidadesPickeadas ?? 0
      const sep = d?._sum?.unidadesSeparadas ?? 0
      return {
    ...
    const responseData = { resumen: result, detalle: detalleResult }
    ...
    console.error('Error por-canal:', error)
